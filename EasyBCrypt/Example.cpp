...
//	if (auto out = std::get_if<NTSTATUS>(&optDerivedKey))
//	{
//		NTSTATUS status = *out;
//		std::cout << "Failed to key from PBKDF2. Exiting..." << std::endl;
//		EasyBCrypt::ReportError(status);
//		epilouge(hAlg, hKdf);
//		ExitProcess(1);
//	}
//	
//	dKey = std::get<std::vector<BYTE>>(std::move(optDerivedKey));
//
//
//	std::vector<BYTE> pbBlob;
//	auto optPbBlob = EasyBCrypt::CreateSymmetricKeyBlob(hAlg, dKey, BCRYPT_CHAIN_MODE_CBC);
//	if (auto out = std::get_if<NTSTATUS>(&optPbBlob))
//	{
//		NTSTATUS status = *out;
...
//	}
//
//	pbBlob = std::get<std::vector<BYTE>>(std::move(optPbBlob));
//
//
//	auto optIV = EasyBCrypt::GenerateIV(BCRYPT_CHAIN_MODE_GCM);
//	if (auto out = std::get_if<NTSTATUS>(&optIV))
//	{
//		NTSTATUS status = *out;
...
//	}
//
//	IV = std::get<std::vector<BYTE>>(std::move(optIV));
//
//	auto optCiphertext = EasyBCrypt::Encrypt(hAlg, pbBlob, IV, plaintext);
//	if (auto out = std::get_if<NTSTATUS>(&optCiphertext))
//	{
//		NTSTATUS status = *out;
...
//	}
//
//	ciphertext = std::get<std::vector<BYTE>>(std::move(optCiphertext));
//
//	std::cout << "\n[+] Ciphertext: " << std::endl;
//
...
//	if (auto out = std::get_if<NTSTATUS>(&optDeciphertext))
//	{
//		NTSTATUS status = *out;
...
//	}
//
//	decipherText = std::get<std::string>(std::move(optDeciphertext));
//
//	std::cout << "\n[+] Plaintext: " << std::endl;
//
